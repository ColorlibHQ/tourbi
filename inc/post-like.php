...
add_action( 'wp_enqueue_scripts', 'tourbi_sl_enqueue_scripts' );
function tourbi_sl_enqueue_scripts() {

	wp_enqueue_script( 'tourbi-post-likes', TOURBI_DIR_JS_URI . 'post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'tourbi-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'tourbi' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_tourbi_process_simple_like', 'tourbi_process_simple_like' );
add_action( 'wp_ajax_tourbi_process_simple_like', 'tourbi_process_simple_like' );
function tourbi_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'tourbi' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'tourbi' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'tourbi' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'tourbi' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'tourbi' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'tourbi' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'tourbi_show_user_likes' );
add_action( 'edit_user_profile', 'tourbi_show_user_likes' );
function tourbi_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'tourbi' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'tourbi' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
